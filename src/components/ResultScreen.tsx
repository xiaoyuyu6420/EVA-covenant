    ...
    >
      <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-white/10">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[0.68rem] text-[#777] tracking-[0.18em]" style={{ fontFamily: "var(--font-tech)" }}>
            {top.isSpecial ? "特殊型" : top.isBoundary ? "边界型" : "匹配"}
          </span>
        </div>
      </div>
        ...
        >
          ...
          >
            <div className="mb-4">
              ...
              <p className="mt-2 text-[0.92rem] leading-[1.7] text-[#d6d6d6]" style={{ fontFamily: "var(--font-title)" }}>
                你是通过朋友的分享链接进来的。测完后可以看到你们两个人的编队对比——是同档回声、同轴支援，还是反差编队。
              </p>
            </div>
            ...
            <button
              onClick={shareReturn}
              className="mt-4 w-full h-10 border border-white/15 text-[0.62rem] tracking-[0.14em] text-[#aaa] flex items-center justify-center gap-1.5 transition-colors hover:text-white hover:border-white/30"
              style={{ fontFamily: "var(--font-tech)" }}
            >
              <Share2 size={13} aria-hidden="true" />
              {returnCopied ? "已发送" : "把结果回传给上一站"}
            </button>
          </div>
        </motion.section>
      ) : null}
